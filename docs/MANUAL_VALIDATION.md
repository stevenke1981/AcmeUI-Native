...
3. Trigger real device loss using **one** of:
   ...
   - **Future debug hotkey**: only if a documented in-app ?�simulate device lost??     hotpath exists in the build under test ??do not invent one for this
...
5. Commit a **繁�?** character/word from candidates; confirm committed text is
   correct and editable.
6. Backspace: delete by **grapheme** (one �?per backspace when appropriate),
...
- Do not say ?�注??works?? ?�IME complete?? or ?�manually validated TC IME??in
...
| Commit | 繁�? commit lands in field as one logical edit | Garbage, missing chars, or wrong target widget |
...
- `status/STATUS.md` — "Automated Only (manual still pending)"
- `status/todos.md` — open manual 注音 item; surface recovery automated vs manual
- `guides/TEXT_AND_IME.md` — product rules for preedit/caret/commit
