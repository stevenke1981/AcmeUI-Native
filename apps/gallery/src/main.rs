...
use acme_widgets::{
    ...
    TableRow, TreeNode, WidgetKey, WidgetNode, breadcrumb, button, card, column, datagrid, label,
...
impl Gallery {
    ...
    fn foundations_page(&self) -> WidgetNode<GalleryMessage> {
        match self.selected_page {
            5 => self.style_page(),
            _ => {
                let name = CATEGORIES[0].pages[self.selected_page.min(5)];
    ...
    fn style_page(&self) -> WidgetNode<GalleryMessage> {
        use acme_layout::Length as L;
        use acme_widgets::label_builder;
            ...
            .child(
                card()
                    ...
                    .child(
                        column()
                            .gap(8.0)
                            .child(label_builder("Accent Card").font_size(16.0).text_foreground().font_weight(acme_style::FontWeight::Bold).build())
        ...
        column()
            .gap(20.0)
            .padding(16.0)
            .child(label_builder("Style & Tailwind API").font_size(22.0).font_weight(acme_style::FontWeight::Bold).build())
